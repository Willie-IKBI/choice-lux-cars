-- Final fix for voucher RPC function - remove problematic ORDER BY
...
-- Create the main voucher data function without ORDER BY
...
BEGIN
    ...
    -- Get transport details for the job - REMOVED ORDER BY
    SELECT COALESCE(
        json_agg(
            json_build_object(
...
COMMENT ON FUNCTION get_voucher_data_for_pdf(int) IS 'Fetches complete voucher data for PDF generation. Fixed pickup_time formatting and removed problematic ORDER BY.';
