...
final clientCompletedJobsProvider = FutureProvider.family<List<Job>, String>((
  ref,
  clientId,
) async {
  final jobsData = await SupabaseService.instance.getCompletedJobsByClient(
    clientId,
  );
  return jobsData.map((json) => Job.fromMap(json)).toList();
});
...
final clientRevenueProvider = FutureProvider.family<double, String>((
  ref,
  clientId,
) async {
  return await SupabaseService.instance.getCompletedJobsRevenueByClient(
    clientId,
  );
});

// Provider for client statistics summary
final clientStatsProvider = FutureProvider.family<Map<String, dynamic>, String>(
  (ref, clientId) async {
    final completedJobs = await ref.read(
      clientCompletedJobsProvider(clientId).future,
    );
