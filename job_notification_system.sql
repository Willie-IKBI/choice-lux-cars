...
ALTER TABLE notifications ADD COLUMN notification_type notification_type_enum;
ALTER TABLE notifications ADD COLUMN job_id bigint REFERENCES jobs(id);
...
BEGIN
    ...
    IF admin_users IS NOT NULL THEN
        ...
        LOOP
            INSERT INTO notifications (user_id, body, notification_type, job_id, created_at)
    ...
    IF manager_users IS NOT NULL THEN
        ...
        LOOP
            INSERT INTO notifications (user_id, body, notification_type, job_id_param, created_at)
            VALUES (target_user, message_param, notification_type_param, job_id_param, NOW());
        END LOOP;
    END IF;
...
-- Trigger for job started notification
...
-- Trigger for passenger onboard notification
...
-- Trigger for job completed notification
...
-- 8. Function to check for stalled jobs and notify
...
-- 9. View for unread notifications
...
-- 10. Function to get notifications for a specific user
