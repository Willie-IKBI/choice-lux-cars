-- Driver Flow Table Schema Updates
-- To support granular trip-level tracking and progress management

-- 1. Add new columns to driver_flow table
ALTER TABLE driver_flow ADD COLUMN current_step text DEFAULT 'vehicle_collection';
ALTER TABLE driver_flow ADD COLUMN current_trip_index integer DEFAULT 1;
ALTER TABLE driver_flow ADD COLUMN progress_percentage integer DEFAULT 0;
ALTER TABLE driver_flow ADD COLUMN last_activity_at timestamptz;
ALTER TABLE driver_flow ADD COLUMN job_started_at timestamptz;
ALTER TABLE driver_flow ADD COLUMN vehicle_collected_at timestamptz;
...
CREATE INDEX idx_trip_progress_job_id ON trip_progress(job_id);
CREATE INDEX idx_trip_progress_status ON trip_progress(status);
CREATE INDEX idx_driver_flow_current_step ON driver_flow(current_step);
CREATE INDEX idx_driver_flow_last_activity ON driver_flow(last_activity_at);
