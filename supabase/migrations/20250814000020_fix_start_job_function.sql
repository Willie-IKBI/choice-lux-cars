-- Fix start_job function to properly handle vehicle collection step
-- Applied: 2025-08-14
-- Description: Fix the start_job function to set current_step to 'vehicle_collection' instead of 'pickup_arrival'
...
BEGIN
    ...
    SET 
        ...
        vehicle_time = NOW(),
        current_step = 'vehicle_collection', -- Keep current step as vehicle_collection
    ...
    -- Log the action
    RAISE NOTICE 'Job % started successfully by driver %. Vehicle collected: true, Current step: vehicle_collection', 
...
COMMENT ON FUNCTION start_job(bigint, numeric, text, numeric, numeric, numeric) IS 'Starts a job by updating driver_flow with vehicle collection data';
...
DO $$
BEGIN
    RAISE NOTICE 'start_job function updated successfully. Vehicle collection step will be properly handled.';
END;
$$;
