-- Progress Management System Migration
-- Applied: 2025-08-11
-- Description: Implements job status enum, driver activity tracking, and progress monitoring
...
-- 2. Update jobs table to use enum (if not already using it)
-- Note: This is a safe operation that won't break existing data
DO $$ 
BEGIN
    -- Check if job_status column exists and is not already an enum
    IF EXISTS (
...
EXCEPTION
    WHEN OTHERS THEN
        -- If conversion fails, leave as is and log the error
        RAISE NOTICE 'Could not convert job_status to enum: %', SQLERRM;
END $$;
...
DROP TRIGGER IF EXISTS trigger_update_job_status ON driver_flow;
