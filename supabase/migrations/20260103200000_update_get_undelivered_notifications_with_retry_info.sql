-- Migration: Update get_undelivered_notifications to include retry_count and last_attempt_at
-- Date: 2026-01-03
-- Purpose: Add retry control fields to support max retries and cooldown logic

BEGIN;

-- Drop and recreate function with new return type
DROP FUNCTION IF EXISTS public.get_undelivered_notifications(integer);

-- Create RPC function to get undelivered notifications efficiently with retry info
...
COMMENT ON FUNCTION public.get_undelivered_notifications(integer) IS
  'Returns undelivered notifications (no successful delivery log entry) sorted by newest first, including max_retry_count and last_attempt_at for retry control. Used by push-notifications-poller Edge Function.';
