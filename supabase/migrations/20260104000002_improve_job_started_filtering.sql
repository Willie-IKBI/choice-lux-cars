-- Migration: Improve Job Started Filtering in Deadline Notifications RPC
-- Date: 2026-01-04
-- Purpose: Make job_started_at filtering more explicit and handle missing driver_flow rows clearly
--          Option A: Treat missing driver_flow as "not started yet" (safer - prevents false negatives)

BEGIN;

-- Update RPC function with explicit handling for missing driver_flow rows
...
BEGIN
  ...
  jobs_with_driver AS (
    ...
    -- Explicit handling: 
    --   - If driver_flow row exists and job_started_at IS NOT NULL -> exclude (job started)
    --   - If driver_flow row doesn't exist -> include (job not started yet)
    --   - If driver_flow row exists but job_started_at IS NULL -> include (job not started)
    SELECT 
      j.id as job_id,
      j.job_number,
      j.driver_id,
    ...
    WHERE 
      ...
      AND (df.job_started_at IS NULL)  -- Job hasn't started (handles both missing row and NULL value)
...
'Finds jobs needing start deadline notifications. Returns jobs where driver has not started job (driver_flow.job_started_at IS NULL, including when driver_flow row is missing) and we are 90 minutes before pickup (manager) or 60 minutes before pickup (administrator/super_admin). Includes manager_id for proper recipient scoping.';
